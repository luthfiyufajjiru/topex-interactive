  ...
  const steps: { id: WorkflowStep; label: string; sub: string; icon: React.ReactNode; disabled: boolean }[] = [
    {
      ...
      sub: recordCount > 0 ? `${recordCount.toLocaleString()} soundings` : 'Bounding Box & Data Stream',
      icon: <Layers size={18} />,
      disabled: false,
    },
    {
      ...
      sub: recordCount > 0 ? (hasGravity ? 'Density & Slab Correction' : 'Requires Gravity Anomaly') : 'Extract soundings first',
    ...
    },
    {
      ...
      sub: 'Anomaly Maps & Modeling Export',
